    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-6 space-y-4">
          <div className="relative w-full h-72 sm:h-96 lg:h-[420px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-md dark:shadow-2xl">
            <Image
            ...
            />
            <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex flex-wrap gap-2 max-w-[90%]">
              {product.is_original ? (
                <span className="bg-emerald-600/95 text-white text-[11px] sm:text-xs font-black px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg shadow-lg uppercase tracking-wider flex items-center gap-1.5 backdrop-blur-sm">
                  <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> %100 Orijinal OEM
                </span>
              ) : (
                <span className="bg-amber-600/95 text-white text-[11px] sm:text-xs font-black px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-lg shadow-lg uppercase tracking-wider flex items-center gap-1.5 backdrop-blur-sm">
                  <Wrench className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> Muadil Parça
                </span>
              )}

              {discountPercent > 0 && (
                <span className="bg-gradient-to-r from-red-600 to-orange-600 text-white text-[11px] sm:text-xs font-black px-2.5 py-1 rounded-md shadow-lg uppercase">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div>
            <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-orange-600 dark:text-orange-400 uppercase tracking-wider mb-2">
              <span>{product.brand}</span>
              <span>•</span>
              <span className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-mono px-2 py-0.5 rounded text-[11px]">
                OEM: {product.part_number}
              </span>
              {product.oem_reference_number && (
                <span className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-mono px-2 py-0.5 rounded text-[10px]">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mt-3">
              ...
              <span className="text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                <CheckCircle2 className="w-3.5 h-3.5 shrink-0" /> Stokta Var ({product.stock} Adet)
              </span>
            </div>
          </div>

          {/* Pricing Box */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-6 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
            <div>
              ...
              <span className="text-[11px] text-slate-500 dark:text-slate-400 block mt-1">KDV Dahil + DHL Express Kargo</span>
            </div>
