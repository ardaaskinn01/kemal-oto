...
export class EmailService {
   ...
   */
  async sendOrderConfirmation(order: any, targetEmail?: string) {
    const email = targetEmail || order.contact_info?.email || order.email || order.shipping_address?.email;
    if (!email) {
      console.warn('sendOrderConfirmation skipped: No email found');
      return null;
    }
    const fullName = order.shipping_address?.full_name || order.full_name || 'Değerli Müşterimiz';
    const origin = process.env.NEXT_PUBLIC_APP_URL || 'https://www.onlinehizliparca.com';
    ...
    try {
      const htmlContent = `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
          <div style="background: #020617; padding: 24px; text-align: center; color: white; border-bottom: 3px solid #fbbf24;">
            <h1 style="margin: 0; font-size: 24px; font-weight: 800; letter-spacing: -0.5px;">ONLINE HIZLI <span style="color: #fbbf24;">PARÇA</span></h1>
            <p style="margin: 4px 0 0 0; font-size: 12px; color: #94a3b8;">Otomotiv Yedek Parça • Bornova / İzmir (Kemal Oto)</p>
          </div>
          ...
          <div style="padding: 24px;">
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 16px; border-radius: 8px; margin: 20px 0;">
              ...
              <p style="margin: 6px 0; font-size: 13px;"><strong>Sipariş No:</strong> #${order.id ? order.id.slice(0, 8) : 'ORD-98241'}</p>
              <p style="margin: 6px 0; font-size: 13px;"><strong>Toplam Tutar:</strong> <span style="font-weight: bold; color: #d97706;">${order.total_amount || 0} TRY</span></p>
            ...
            <div style="background: #fffbeb; border-left: 4px solid #fbbf24; padding: 12px; border-radius: 4px; margin: 20px 0; font-size: 13px; color: #92400e; line-height: 1.5;">
            ...
            <p style="margin-top: 24px; font-size: 12px; color: #64748b; border-top: 1px solid #f1f5f9; padding-top: 16px;">
              Müşteri Hizmetleri: <strong>0542 292 44 92</strong> | <a href="mailto:info@onlinehizliparca.com" style="color: #d97706; text-decoration: none;">info@onlinehizliparca.com</a>
            </p>
          </div>
        </div>
      `;
      ...
      const data = await this.resend.emails.send({
        from: fromEmail,
        to: email,
        subject: `Sipariş Onayı #${order.id ? order.id.slice(0, 8) : 'ORD-98241'} - Online Hızlı Parça`,
        html: htmlContent,
      });
  ...
  async sendShippingNotification(
    ...
    origin = 'https://www.onlinehizliparca.com',
    carrier = 'DHL Express'
  ) {
    const email = order.contact_info?.email || order.email;
    if (!email) {
      console.warn('sendShippingNotification cancelled: Customer email missing');
      return null;
    }
    ...
    try {
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
          ...
          <div style="padding: 24px;">
            ...
            <p style="font-size: 14px; line-height: 1.6; color: #334155;">
              <strong>#${order.id ? order.id.slice(0, 8) : 'ORD-98241'}</strong> numaralı siparişiniz depomuzda kontrol edilip özenle paketlendi ve <strong>${carrier}</strong> kuryesine elden teslim edildi.
            </p>

            <div style="background-color: #fffbeb; border: 2px solid #fbbf24; border-radius: 12px; padding: 20px; margin: 24px 0; text-align: center;">
              <p style="margin: 0 0 8px 0; font-size: 12px; color: #92400e; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px;">
              ...
              <p style="margin: 10px 0 0 0; font-size: 12px; color: #64748b;">
                ...
                <a href="${dhlOfficialUrl}" style="color: #d97706; word-break: break-all; font-size: 11px; text-decoration: underline;">
                  ${dhlOfficialUrl}
                </a>
              </p>
            </div>
            ...
            <div style="background: #f8fafc; border: 1px solid #e2e8f0; padding: 12px; border-radius: 8px; font-size: 12px; color: #475569; line-height: 1.5; text-align: center;">
              Kargo takip numaranız: <strong style="font-family: monospace; color: #0f172a;">${trackingNumber}</strong> | Dilerseniz sitemizdeki <a href="${internalTrackingUrl}" style="color: #d97706; font-weight: bold;">Siparişlerim</a> sayfasından da takip edebilirsiniz.
            </div>

            <p style="margin-top: 24px; font-size: 12px; color: #64748b; border-top: 1px solid #f1f5f9; padding-top: 16px; text-align: center;">
              Online Hızlı Parça Müşteri Hizmetleri: <strong>0542 292 44 92</strong> | <a href="mailto:info@onlinehizliparca.com" style="color: #d97706; text-decoration: none;">info@onlinehizliparca.com</a>
            </p>
          </div>
        </div>
      `;
      ...
      const data = await this.resend.emails.send({
        from: fromEmail,
        to: email,
        subject: `Siparişiniz Kargoya Verildi (Takip No: ${trackingNumber}) - Online Hızlı Parça`,
        html: htmlContent,
      });
